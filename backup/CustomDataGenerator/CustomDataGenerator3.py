...
class CustomDataGenerator(Sequence):
    ...
    def __next__(self):
        ...
        index = getattr(self, '_index', 0)  # Get the current index from the object attribute or default to 0
        #print('LENNNNN', len(self))
        while index < len(self):
            batch = self[index]
            index += 1
            setattr(self, '_index', index)  # Update the index in the object attribute
            return batch
        # If the index exceeds the length of the dataset, raise StopIteration
        raise StopIteration
...
data_generator = create_data_generator(directory_img, training_labels, batch_size, image_shape)
