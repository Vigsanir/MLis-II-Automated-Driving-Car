...
def create_data_generator2(input_path, labels_df, batch_size, image_shape, output_label='speed', augmentations=None, shuffle=True ):  # if we don't specify the output_label, by default the output_label is speed.
    return CustomDataGenerator2(input_path, labels_df, batch_size, image_shape, output_label, augmentations=None, shuffle=True)
...
def test_batch2(batch_images, batch_labels, output_label):
    print("Batch Images:")
    for image in batch_images:
        print("[")
        for row in image:
...
for epoch in range(epochs):
    ...
    for batch_images, batch_labels in data_generator:
       ...
       # print('AAAAAAAAAAAAAAAAAAA', batch_labels)
        test_batch2(batch_images, batch_labels, output_label)